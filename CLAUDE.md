...
- **`Module`** — holds a `base.ConfigurableModule` (+ its `Extend()` view for typed config reads) plus immutable **host-policy** fields that are *not* config keys: `enabled`, `allow`, `envKeys` and `allowAll`. They are set only in Go and can never be read from a script or an env var.
- **Constructors** — `NewModule()` (disabled, default config), `NewModuleWithConfig(...)` (disabled, preset config), `NewModuleWithAllow(allow ...)` (**enabled** with an allowlist). `NewModuleWithPolicy(Policy{Commands, EnvKeys})` adds copied exact environment-key grants; `NewModuleWithAllow` grants no environment overrides. There is deliberately no allow-*setter* method: grants are bound at construction.
...
- **`run` (the gate)** — the ordered pipeline: unpack args → require non-empty command → `sanitizeCommand` (UTF-8 + reject control/format chars) → split to argv via `creachadair/shell` (with a Windows backslash-doubling workaround) → join to a *canonical* string → `commandAllowed` check → resolve config defaults → validate every merged environment key against host policy → `executeArgv`. The disabled-module check is the **first** thing `run` does.
- **`which` (`starWhich`)** — `exec.LookPath` only; returns the path or `None`. It does **not** execute, so it works even on a disabled module.
- **`executeArgv`** — the actual `exec.CommandContext` call (timeout from the thread context), env assembly (minimal host environment + approved config/call values), stdin wiring, and the stdout/stderr capture matrix driven by `capture_output`/`combine_output`/`realtime_output`. Returns a `ProcessResult`.
...
6. **Backward compatibility.** Config defaults reproduce historical behavior (`capture_output` defaults true; `timeout` 0 = no limit). The enable-gate is the one *new* lever and it defaults to off, so a host that does not grant execution gets the safe (disabled) default. Restricted scripts that supply environment overrides must migrate to `NewModuleWithPolicy` with explicit keys; trusted allow-all retains its environment behavior.
